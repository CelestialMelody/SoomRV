Hi [@mathis-s](https://github.com/mathis-s), I'm currently learning SoomRV and encountered some issues with Linux boot and COSIM. I would appreciate your guidance on these problems.

## Building Linux

### Issue Description

When running `make linux` on Arch Linux, I encountered a compilation error with `host-m4-1.4.19` during the build of buildroot-2023.05-rc2. The newer GCC version (15.2.1) has compatibility issues with the `_GL_ATTRIBUTE_NODISCARD` attribute used in the older m4 code.
...
To address this, I tried using a newer buildroot version to build the Linux image. Here's my modified Makefile:
...
# Kernel and Busybox configuration (unchanged unless you want to use defaults)
...
The build produced numerous warnings followed by errors:
...
I then modified the Makefile to use buildroot's default configuration:
...
# Generate .config rule
# Instead of copying old files, let Buildroot generate a standard RISC-V 32-bit default configuration
# Use qemu_riscv32_virt_defconfig as the base
$(BUILDROOT_DIR)/.config: $(BUILDROOT_DIR)
...
1. The `buildroot.config`, `kernel.config`, and `busybox.config` files start with "Automatically generated file". I'd like to understand how these configuration files are generated. (Also, if possible, I'd love to know how `device_tree.dts` is generated, though that's outside the scope of this issue.)

2. The `.config` generated by `make qemu_riscv32_virt_defconfig` likely needs additional configuration. For example, the Target ABI is `ilp32d`, while the repository's `buildroot.config` uses `ilp32`. (However, I haven't focused on this since Linux runs fine without COSIM.)

3. Are `kernel.config` and `busybox.config` definitely required? Can I use the defaults? What are the differences from defaults? The default `kernel.config` seems related to `buildroot/arch/Config.in.riscv`, and `busybox.config` has a default file at `package/busybox/busybox.config`.

---

## COSIM Issue

After building the image using the modified Makefile above, I attempted to run Linux with COSIM enabled:
...
In `Simif.cpp`, I tried filtering `CSR_TINFO` before `is_pass_thru_inst`. I found that adding `case CSR_TINFO` to the `switch (csrID)` in `is_pass_thru_inst` didn't resolve the issue. I then modified the original ERROR 6 logic to ignore `CSR_TINFO` checks:

```cpp
// Simif.cpp: func cosim_instr

// 1. Force sync: For CSR TINFO (0x7a4) (testing only)
// Directly check instruction bits, don't rely on is_pass_thru_inst, don't check flags
// 0x7a4 is the CSR address, in bits 31-20 of the instruction
if (((inst.inst >> 20) & 0xFFF) == CSR_TINFO)
{
    ...
    if (inst.rd != 0) {
        // Force RTL register value to Spike
...
// 2. Modified original ERROR 6 logic to ignore CSR_TINFO checks (testing only)
...
{
    // If this is TINFO (0x7a4) instruction, we allow instret mismatch
    if (((inst.inst >> 20) & 0xFFF) == CSR_TINFO)
    {
        // Optionally print a warning, or completely ignore
        // fprintf(stderr, "Warn: Ignoring TINFO instret mismatch. RTL=%lu, Spike=%lu\n", inst.minstret, spike_minstret);
    }
    else
    {
        // For other instructions, still error out
...
However, this introduced a new error:
...
Since `0x342` is `CSR_MCAUSE`, I tried applying a similar force-sync approach for the PC, but this led to even more errors.

### Workaround

However, if I disable COSIM, Linux boots successfully:
...
I don't know how to properly support COSIM. Could you share your thoughts on this? (Of course, I'm still studying your code.)

---

**Additional Notes:**

- **Environment:** Arch Linux with GCC 15.2.1
- **Buildroot versions tried:** 2023.05-rc2 and 2025.11
- **SoomRV version:** Latest from main branch
- **COSIM status:** Works when disabled, fails with various errors when enabled

Any guidance would be greatly appreciated. Thank you for your time and this excellent project!
