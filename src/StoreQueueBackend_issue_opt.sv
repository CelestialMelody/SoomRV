...
always_ff@(posedge clk /*or posedge rst*/) begin
    ...
    if (rst) begin
        ...
        if (OUT_uopSt.valid && !IN_stallSt) begin
            // Consume the issued op once the downstream path accepts it.
            // Under backpressure we keep OUT_uopSt stable instead of reissuing every other cycle.
            OUT_uopSt <= ST_UOp'{valid: 0, default: 'x};
        end
