// BranchPredictor variant with explicit BT-update arbitration.
...
// Branch target updates can come from multiple sources in one cycle.
// Arbitrate with explicit priority and cache non-selected updates.
BTUpdate btUpdate;
BTUpdateArbiter#(.NUM_IN(NUM_IN)) btUpdateArbiter
(
    .clk(clk),
    .rst(rst),
    .IN_updates(IN_btUpdates),
    .OUT_update(btUpdate)
);
