...
always_comb begin
    ...
    for (integer i = 0; i < NUM_OUT; i=i+1) begin
        if (OUT_uop[i].valid &&
            ...
            for (integer j = 0; j < $bits(OUT_uop[i].wmask); j=j+1)
    ...
    for (integer i = 0; i < 2 * NUM_ENTRIES; i=i+1) begin
        ...
        if (entries[ii].addrAvail &&
            ...
            if (entries[ii].loaded) begin
                ...
                for (integer j = 0; j < $bits(entries[ii].wmask); j=j+1)
    ...
    `else
    for (integer i = 0; i < $bits(lookupMaskIter[h][outputIdx]); i=i+1)
...
always_comb begin
    ...
    if (entries[i].addrAvail &&
        ...
        if (entries[i].loaded) begin
            for (integer j = 0; j < $bits(entries[i].wmask); j=j+1)
