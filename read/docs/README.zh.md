...
2. 补充依赖安装：（可以去参考 Dockerfile 如何搭建环境）
   ...
   AUR 源的 slang-verilog PKGBUILD 中的 sha256sum 过期/错误，选择手动构建

   ```
   cd /opt
   sudo wget https://github.com/MikePopoloski/slang/archive/refs/tags/v9.1.tar.gz -O slang.tar.gz
   sudo tar -xzf slang.tar.gz
   cd slang-9.1
   sudo cmake -B build -DCMAKE_INSTALL_PREFIX=/opt/slang
